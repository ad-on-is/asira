...
const crfQualityValues = [18, 19, 20, 21, 22, 23, 24, 25, 26];
...
function updateAudioOptions() {
  execAsync(["bash", "-c", `pactl list sources | grep -E "Name:|Description"`])
    .catch((error) => {
      print(error);
    })
    .then((value) => {
      ...
      for (let i = 0; i < lines.length; i += 2) {
        ...
        if (
          ...
          descriptionLine?.startsWith("Description: ")
        ) {
...
function showNotification(filePath: string, appName: string) {
  execAsync([
    "bash",
    "-c",
    `
      ...
      <box vertical={false}>
        <ScreenshotButton
          ...
          onClicked={() => {
            ...
            const time =
              GLib.DateTime.new_now_local().format("%Y_%m_%d_%H_%M_%S")!;
...
function ScreenRecording() {
  ...
  setTimeout(() => {
    bind(App.get_window(ScreenshotWindowName)!, "visible").subscribe(
      (visible) => {
        if (!visible) {
      ...
      >
        <ScreenshotButton
          ...
          onClicked={() => {
            ...
            const audioParam =
              selectedAudio.get() !== null
                ? `--audio=${selectedAudio.get()!.name}`
                : "";
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <box vertical={true} css="padding: 20px;">
              <ScreenShots />
              <Divider
                css={`
